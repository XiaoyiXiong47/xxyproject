...
# #     orientation.set("zAngle", f"{roll:.1f}" if not np.isnan(roll) else "")
# #     location = ET.SubElement(hand, "location")
# #     ET.SubElement(location, "loc", x=hand_loc[0], y=hand_loc[1], z=hand_loc[2])
# #     if movement:
# #         ET.SubElement(hand, "movement")
# #
# #     return ET.ElementTree(root)
...
def create_orientation_element(parent, ori):
    orientation = ET.SubElement(parent, 'orientation')
    orientation.set('xAngle', str(ori[1]))
    orientation.set('yAngle', str(ori[2]))
    orientation.set('zAngle', str(ori[3]))
...
def add_hand_block(parent, side, frame_idx, all_angles, orientation, location):
    hand = ET.SubElement(parent, 'hand')
    hand.set('side', side)
    angle_dict = build_angle_dict(all_angles[frame_idx])
    create_finger_elements(hand, angle_dict)
    create_orientation_element(hand, orientation[frame_idx])
    create_location_element(hand, location[frame_idx])


def generate_xml(left_angles, right_angles, left_orientation, right_orientation, left_location, right_location,
                 left_seg, right_seg, left_mid, right_mid, dataset, gloss, output_path='output.xml'):
    root = ET.Element('sem')
    root.set('type', 'sign')
    ...
    # First handle right hand segments
    for idx, (start_R, end_R) in enumerate(right_seg):
        seq = ET.SubElement(root, 'seq', number=str(seq_num))

        # Add right hand
        add_hand_block(seq, 'A', start_R, right_angles, right_orientation, right_location)

        movement = ET.SubElement(seq[-1], 'movement')
        for mid_frame in (right_mid[idx] if isinstance(right_mid[idx], list) else [right_mid[idx]]):
            if start_R < mid_frame < end_R:
                mid = ET.SubElement(movement, 'mid')
                add_hand_block(mid, 'A', mid_frame, right_angles, right_orientation, right_location)

        end_tag = ET.SubElement(movement, 'end')
        add_hand_block(end_tag, 'A', end_R, right_angles, right_orientation, right_location)

        # Check if corresponding left hand segment exists and is near in time
        for jdx, (start_L, end_L) in enumerate(left_seg):
            if abs(start_L - start_R) <= 3 and abs(end_L - end_R) <= 3:
                add_hand_block(seq, 'B', start_L, left_angles, left_orientation, left_location)
                movement_L = ET.SubElement(seq[-1], 'movement')
                for mid_frame in (left_mid[jdx] if isinstance(left_mid[jdx], list) else [left_mid[jdx]]):
                    if start_L < mid_frame < end_L:
                        mid = ET.SubElement(movement_L, 'mid')
                        add_hand_block(mid, 'B', mid_frame, left_angles, left_orientation, left_location)

                end_tag = ET.SubElement(movement_L, 'end')
                add_hand_block(end_tag, 'B', end_L, left_angles, left_orientation, left_location)
                break

        seq_num += 1

    # Then handle unmatched left hand segments
    for idx, (start_L, end_L) in enumerate(left_seg):
        matched = any(abs(start_L - start_R) <= 3 and abs(end_L - end_R) <= 3 for start_R, end_R in right_seg)
        if matched:
            continue
        ...
        movement = ET.SubElement(seq[-1], 'movement')  # under last added hand
        for mid_frame in (left_mid[idx] if isinstance(left_mid[idx], list) else [left_mid[idx]]):
            if start_L < mid_frame < end_L:
                mid = ET.SubElement(movement, 'mid')
                add_hand_block(mid, 'B', mid_frame, left_angles, left_orientation, left_location)
