...
def compute_velocity(trajectory):
    # trajectory: Nx3
    diffs = np.diff(trajectory, axis=0) # velocity vectors
    speed = np.linalg.norm(diffs, axis=1) # Scalar speed
    return diffs, speed
...
def segment_signs_from_velocity_and_shape(
    ...
    def find_velocity_minima(v):
        from scipy.signal import argrelextrema
        minima = argrelextrema(v, np.less, order=10)[0] # order决定了在几个neighbor之间找最小值
        return minima.tolist()

    def find_velocity_maxima(v):
        maxima = argrelextrema(v, np.greater, order=10)[0]
    ...
    if velocity is None or len(velocity) == 0 or np.size(velocity) == 0:
        return [], []
    v_min = find_velocity_minima(velocity)
    v_max = find_velocity_maxima(velocity)
    keyframes = merge_close_keyframes(sorted(set(v_min + v_max)), min_gap=5)
...
def process_hand_landmarks(left_wrist, right_wrist, left_angles, right_angles):
    ...
    if isinstance(left_wrist, list) and len(left_wrist) > 0:
        ...
        if left_wrist_array.ndim == 2 and left_wrist_array.shape[1] == 3:
            ...
            left_velocity, left_speed = compute_velocity(left_wrist_trajectory)
            ...
            print("left_key_frames:", left_key_frames)
        else:
    ...
    if isinstance(right_wrist, list) and len(right_wrist) > 0:
        ...
        if right_wrist_array.ndim == 2 and right_wrist_array.shape[1] == 3:
            ...
            right_velocity, right_speed = compute_velocity(right_wrist_trajectory)
            ...
            print("right_key_frames:", right_key_frames)
        else:
